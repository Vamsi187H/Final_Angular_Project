...
export class LoginComponent implements OnInit {
    ...
    if (this.email.length == 0 || this.password.length == 0)
      ...
      this.log.login(this.email, this.password).subscribe({
        next: (response: any) => {
          ...
          else this.route.navigate(['user/home']);
        },
  ...
  register(): void {
    ...
    if (
      this.rusername.length == 0 ||
      this.rpassword.length == 0 ||
      this.vpassword.length == 0
    )
    ...
    else if (!emailregex.test(this.remail)) this.rerror = 'not in email format';
    else {
      this.rstatus = true;
      ...
      this.log.register(obj).subscribe({
        next: (result: any) => {
          if (result.success == false) alert('User already exists');
          else {
